...
class IBMSVCSafeguardedPolicy:
    ...
    def __init__(self):
        ...
        self.backup_unit = self.module.params.get('backupunit', '')
        self.backup_interval = self.module.params.get('backupinterval', '')
        self.backup_start_time = self.module.params.get('backupstarttime', '')
        self.retention_days = self.module.params.get('retentiondays', '')
    ...
    def basic_checks(self):
        if self.state == 'present':
            fields = ['name', 'backup_interval', 'backup_start_time', 'retention_days', 'backup_unit']
            ...
            fields = ['backup_interval', 'backup_start_time', 'retention_days', 'backup_unit']
    ...
    def create_sg_policy(self):
        ...
        cmdopts = {
            'name': self.name,
            'backupstarttime': self.backup_start_time,
            'backupinterval': self.backup_interval,
            'backupunit': self.backup_unit,
    ...
    def sg_probe(self):
        ...
        for field in fields:
            if field == 'backup_start_time':
                updates.append(self.sg_policy_details[field] != '{0}00'.format(getattr(self, field)))
            else:
                updates.append(self.sg_policy_details[field] != getattr(self, field))
